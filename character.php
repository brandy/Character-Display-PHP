...
$variables = array("debug" => (bool)$debug, "gender" => 0, "job" => 0, "skin" => 0, "hair" => 30030, "face" =>  20000);
$mysqli = @mysqli_connect("localhost", "root", "", "ourstoryv144") or die("Connection Error: ".mysqli_connect_error());
if($character = mysqli_fetch_row(mysqli_query($mysqli, "SELECT `id`, `gender`, `job`, `skincolor`, `hair`, `face` FROM `characters` WHERE `name` = '".mysqli_real_escape_string($mysqli, $name)."' LIMIT 1"))) {
...
mysqli_close($mysqli);
$gd = new Controller($debug);

$gd->setConstants($variables)
